...
#     def __init__(self, rsi_window=14, bb_window=20, bb_std=2):
...
class TechnicalAnalysisModel:
    def __init__(self, rsi_window=14, bb_window=20, bb_std=2, macd_fast=12, macd_slow=26, macd_signal=9, stoch_k=14, stoch_d=3):
        self.rsi_window = rsi_window
        self.bb_window = bb_window
        self.bb_std = bb_std
    ...
    def predict(self, data):
        ...
        data['SlowK'], data['SlowD'] = self.calculate_stochastic(data)

        # Generate buy/sell signals based on technical indicators
        data['Signal'] = 0
        data.loc[(data['Close'] > data['BB_Upper']) & (data['RSI'] > 70) & (data['MACD'] > data['MACD_Signal']) & (data['SlowK'] > 80), 'Signal'] = -1
        data.loc[(data['Close'] < data['BB_Lower']) & (data['RSI'] < 30) & (data['MACD'] < data['MACD_Signal']) & (data['SlowD'] < 20), 'Signal'] = 1
