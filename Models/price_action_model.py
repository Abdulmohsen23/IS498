...
class PriceActionModel:
    def __init__(self, window_size=14, n_estimators=100, max_depth=5):
    ...
    def generate_labels(self, data):
        ...
        labels.loc[(data['Higher_High'] == 1) & (data['Higher_High'].rolling(self.window_size).sum() >= 3)] = 1
        labels.loc[(data['Lower_Low'] == 1) & (data['Lower_Low'].rolling(self.window_size).sum() >= 3)] = -1
        return labels
